...
2. A three-storey shear building has equal floor masses, so its mass matrix is $M = mI$. Its storey stiffnesses from the ground up are $6k$, $5k$ and $3k$, which gives the stiffness matrix $K = k\begin{bmatrix} 11 & -5 & 0 \\ -5 & 8 & -3 \\ 0 & -3 & 3 \end{bmatrix}$. Set up the eigenvalue problem $(K - \omega^2 M)\boldsymbol{\phi} = \mathbf{0}$, and find the natural frequencies in terms of $k/m$ and the mode shapes.
...
2. The moisture transfer between the surface, middle and core layers of a drying grain kernel is modelled by $\dfrac{d\mathbf{M}}{dt} = A\mathbf{M}$, with $t$ in hours and $A = \begin{bmatrix} -1.1 & 0.5 & 0 \\ 0.5 & -0.8 & 0.3 \\ 0 & 0.3 & -0.3 \end{bmatrix}$. Find the eigenvalues of $A$ and explain how the smallest eigenvalue in magnitude controls the drying time.
