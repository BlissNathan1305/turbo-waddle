...
1. For a van der Waals gas, $\left(P + \dfrac{a}{V^2}\right)(V - b) = RT$. Find $\left(\dfrac{\partial P}{\partial V}\right)_T$ and $\left(\dfrac{\partial P}{\partial T}\right)_V$. Verify the cyclic relation $\left(\dfrac{\partial P}{\partial V}\right)_T\left(\dfrac{\partial V}{\partial T}\right)_P\left(\dfrac{\partial T}{\partial P}\right)_V = -1$.
...
1. The pressure distribution around a producing well is $p(x, y) = p_e - \dfrac{q\mu}{4\pi kh}\ln\dfrac{x^2 + y^2}{r_e^2}$. Find $\nabla p$, and show that the flow (along $-\nabla p$) is directed radially towards the well.
2. The oil formation volume factor $B_o(p, T)$ is approximated by $B_o = 1.2 + 0.0001(T - 150) - 0.00002(p - 3000)$. Find the linear (Taylor) approximation of $B_o$ about $(p, T) = (3000, 150)$, and estimate $B_o$ at (2800 psi, 160 °F).
